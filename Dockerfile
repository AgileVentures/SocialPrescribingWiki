...
RUN apt-get update -qq && apt-get install -y software-properties-common apt-transport-https gnupg && \
    ...
    apt-get update -qq && apt-get install -y nodejs parsoid wget zip --allow-unauthenticated --no-install-recommends && \
    mv ./images ./images-old && ln -s /storage/images ./ && rm -rf /var/lib/apt/lists/*
